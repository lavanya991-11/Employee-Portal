...
function SystemSettings() {
    ...
    // Load shared company settings (logo + name) so admins see the current values.
    useEffect(() => {
        settingsApi.get().then(({ data }) => {
            const s = data.settings || {};
            setCompanyName(s.companyName || '');
            if (s.companyLogo) {
                setLogoUrl(s.companyLogo);
    ...
    const onUploadLogo = async () => {
        ...
        try {
            ...
            await settingsApi.update({ companyLogo: url }); // share app-wide
